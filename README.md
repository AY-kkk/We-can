...
- 🔐 **完整鉴权**：注册 / 登录 / 刷新 / 登出 / 找回密码，JWT（access+refresh）+ bcrypt 哈希，路由守卫。
- 🛡️ **管理员后台**：数据看板（Recharts）+ 用户管理（搜索/分页/启用禁用/改角色/重置密码/删除），前后端双重校验。
- 🎭 **人格化笔面准备**：产品/运营/算法/市场/前端**方向切换器**，切换后加载对应智能体人格（Echo / Nova / 全栈大师 / 前端设计师）。
- 📚 **题库量达标**：每方向 **116 题**（笔试 / 技术面 / 行为面 STAR / 业务面 四类，合计 580+）。
- 🔗 **真实经验帖**：每方向 **85+ 条**、5 平台来源（掘金 / CSDN / 牛客 / 人人都是产品经理 / 小红书），**全部经 HTTP 200 可达性校验、可点击打开、无死链**（合计 425+）。
