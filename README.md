...
- 🔗 **真实经验帖**：每方向 **85 条**、多平台来源（掘金 / CSDN / SegmentFault 思否 / 人人都是产品经理 / 小红书），其中 **小红书 30 条/方向**（搜索深链，打开即为该方向真实笔记合集）。全部经 **HTTP 200 可达性校验、可点击打开、无死链**（7 方向合计 595）。
- 🖼️ **seeddream 品牌素材**：吉祥物 / 空态 / 错误态 / 头图，均由 `ark-cli seeddream` 生成并落地到前端。
- 🧩 **数据隔离**：用户只见自己的简历 / 复盘 / 清单 / 收藏。
- 🔔 **交互反馈闭环**：全局 Toast 提示（成功/失败/信息）贯穿收藏、清单、润色、复盘、模拟面试等操作；经验帖支持**按来源筛选**（全部 / 小红书 / 掘金 / CSDN / …）。
...
| 经验帖集合 | 多源真实经验帖（85/方向，含小红书 30/方向）+ 方向筛选 + 搜索 + 收藏，均带原文链接 |
